    ...
    <footer className="relative bg-main-50 overflow-hidden text-white   ">
      <div className="container  grid grid-cols-1 md:grid-cols-6 gap-3 relative p-10 padding-container">
        ...
        <div className="col-span-1 md:col-span-2 flex flex-col items-center md:items-start z-10">
          ...
          />
          <h3 className="text-sm font-bold mb-4 text-[white]">House of Prayer Ministries for All Nations</h3>
        </div>

        {/* Navigation Links */}
        <div className="col-span-1 md:col-span-1 z-10 text-[white]">
          <h4 className=" mb-4 border-b-4 border-yellow-50 inline-block pb-1">Navigation</h4>
          <ul>
        ...
        {/* Social Media Links */}
        <div className="col-span-1 md:col-span-1 z-10 text-[white]">
          <h4 className=" mb-4 border-b-4 border-yellow-50 inline-block pb-1">Social Media</h4>
          <ul>
            {socialMediaLinks.map(social => (
              <li key={social.platform} className="mb-2">
                <a href={social.href} className="hover:underline">{social.platform}</a>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="col-span-2 md:col-span-2 z-10 text-[white]">
          <div className="p-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <h4 className="  mb-4 border-b-4 border-yellow-500 inline-block pb-1">Headquarter Contact</h4>
                <p>{headquarters.address}</p>
                <a href={`mailto:${headquarters.email}`} className="underline">{headquarters.email}</a>
                <p className="font-bold mt-2">{headquarters.phone}</p>
              </div>

              {/* Lagos Branch Contact */}
              <div>
              <h4 className=" text-sm mb-4 border-b-4 border-yellow-500 inline-block pb-1">Lagos Branch Contact</h4>
                <p>{lagosBranch.address}</p>
                <a href={`mailto:${lagosBranch.email}`} className="underline">{lagosBranch.email}</a>
