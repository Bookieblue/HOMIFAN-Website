      ...
      <div className="max-container px-4">
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8">
          ...
          <div className="w-full lg:w-[20%] relative bg-[#EDEAFE] rounded-lg p-8 shadow-lg">
            <h3 className="text-lg text-purple-50 font-semibold mb-4">OUR SERVICES</h3>
            <ul>
              {servicesData.map((service, index) => (
                <li key={index} className="mb-6">
                  <h4 className="text-lg font-bold flex gap-3"><Image src='/chevron-down.svg' width={10} height={10} alt="chevron" />{service.title}</h4>
            ...
            <Image src='/quote2.svg' width={30} height={10} alt="chevron" className='absolute right-8 top-8' />
          </div>

          {/* Right Side - Branches */}
          <div className="w-full lg:w-[80%]">
          <p className='mb-7 text-[white] text-xl'>You can't afford to miss a wonderful service at any of our churches.</p>
            <div className="gap-6 flex flex-col lg:flex-row ">
            {branchData.map((branch, index) => (
              <div key={index} className='border p-2 rounded-lg'>
              <div key={index} className="w-full bg-[#FEEBD7] text-black-50 rounded-lg p-8 shadow-lg">
                <h4 className=" text-[#161722]">{branch.title}</h4>
                <p className="text-xl font-bold mt-4">
                  <span className="text-sm font-normal text-[#161722]">Address</span>
                  <br />
                  {branch.address}
                </p>
                ...
                <button className="border-b border-black-50 gap-3 text-white font-semibold mt-6 px-2 py-1 flex items-center">
                  <Image src='/whatsap-icon.svg' width={20} height={10} alt="chevron" /> {branch.buttonText} 
                </button>
                </div>
